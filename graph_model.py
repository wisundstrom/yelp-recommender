...
def biz_preference_state(driver, user_id, biz_id, state):
    ...
    for cat in cat_ids:
        ...
        temp = cypher(
            ...
            WHERE r.id ='{cat}' AND NOT u.id='{user_id}' AND s.name='{state}'\
            RETURN u.id",
            ['u.id'])
...
def user_preference_state(driver, user_id, biz_id, state):
    """This function returns the estimated ratings probability distribution for a specific user and business,
...
def graph_model_predict(driver, user_id, biz_id, state=None):
    ...
    if state:
        biz_pref = biz_preference_state(driver, user_id, biz_id, state)
        user_pref = user_preference_state(driver, user_id, biz_id, state)
...
def biz_preference(driver, user_id, biz_id):
    ...
    for cat in cat_ids:
        # this loop sends a crypher query to retreive users in each category
        temp = cypher(
            driver,
            f"MATCH (u:User)-[]->(r:Reputation)\
            WHERE r.id ='{cat}' AND NOT u.id='{user_id}'\
            RETURN u.id",
            ['u.id'])
        user_in_cat.append(temp)
...
def user_preference(driver, user_id, biz_id):
    """This function returns the estimated ratings probability distribution for a specific user and business,
    limited to ratings, users, and businesses in a the entire dataset. The function takes the driver object for
    the neo4j connection strings of the user and business yelp ids, and the two letter state abbreviation.
    It returns an array of the estimated probability distrubtion prob(user rating of business = k)"""
